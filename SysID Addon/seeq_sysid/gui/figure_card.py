...
class FigureCard(v.Card):
    ...
    def plot(self, train_df: DataFrame, validation_df: DataFrame):
        ...
        for tag in train_df.columns:
            self.train_fig.add_trace(go.Scatter(x=train_df[tag].dropna().index, y=train_df[tag].dropna(), name=tag, visible=True,
        ...
        for tag in validation_df.columns:
            self.validation_fig.add_trace(
                go.Scatter(x=validation_df[tag].dropna().index, y=validation_df[tag].dropna(), name=tag, visible=True,
                           line=dict(color=self.colors[tag_id % 10])))
            tag_id += 1
